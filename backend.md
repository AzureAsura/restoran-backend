...
- 1 `Restaurant` ("Megatha Restaurant & Lounge"), `settings: { hold_time_minutes: 15, tax_rate: 10, service_charge: 5 }`.
...
| 2.4 | 2026-07-19 | **AI Concierge (RAG Chat), backend selesai penuh** — satu-satunya FR yang belum diimplementasi sejak 2.2, sekarang tuntas: `POST /ai/chat` (§8.1) live. RAG retrieval via `vector_store`/pgvector cosine similarity (FR-AI-06), deteksi intent rule-based bilingual ID/EN, availability check + sweep waktu alternatif (FR-AI-01, reuse `findAvailableTable`), customer context by HP (FR-AI-05), rekomendasi menu + FAQ (FR-AI-03/04) dari Gemini JSON-mode dengan guardrail anti-halusinasi (meja/menu/kontak resto selalu dari data asli, gak pernah dikarang). Booking penuh via chat (FR-AI-02, multi-turn, reuse `createBooking()` — realisasi DB selalu jadi sumber teks konfirmasi, bukan klaim AI). Cancel booking **sengaja tidak self-service** via chat (keputusan keamanan — nomor HP/`booking_code` gak cukup kuat buat verifikasi identitas publik), diarahkan ke telepon staff. `vector_store` auto re-index tiap menu di-CRUD (fire-and-forget, gak gantung ke uptime Gemini). Fallback graceful kalau Gemini API gagal/timeout (respons dwibahasa + nomor telepon asli, bukan error mentah). Structured logging per chat call (log-only, gak ada tabel DB baru). Detail lengkap + seluruh temuan implementasi: `backend/plan.md`, `backend/src/modules/ai/docs/ai.service.md`. Widget chat frontend (Fase 13-16) masih belum dikerjakan. | Claude (AI pair programmer) |
